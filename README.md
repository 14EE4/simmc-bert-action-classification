...
이후 3 trials / 2 folds 실행에서 확인된 최적값(`lr=1.9739941119675086e-05`, `weight_decay=0.02266127742232566`, `dropout_rate=0.25346148981751515`)을 기준으로 한 차례 탐색 범위를 좁혔습니다. 다만 해당 설정의 최종 Test Accuracy/Macro F1이 이전 모델 개선-6 기준 결과보다 낮게 나와, 현재 노트북의 탐색 범위는 모델 개선-6 기준 범위로 되돌렸습니다.

| Hyperparameter | 현재 탐색 범위 |
| --- | --- |
| `lr` | `1.8e-5 ~ 2.6e-5` |
| `weight_decay` | `0.01 ~ 0.06` |
| `dropout_rate` | `0.2 ~ 0.4` |

노트북의 실행 셀에서 아래 UI 변수로 탐색 범위를 직접 조정할 수 있습니다.
